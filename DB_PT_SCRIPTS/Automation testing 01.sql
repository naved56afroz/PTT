...
BEGIN
 ...
 RETURN l_sql_id;
END compute_sql_id;
SELECT SQL_HANDLE ,compute_sql_id(sql_text) sql_id, signature,PLAN_NAME
  FROM dba_sql_plan_baselines where compute_sql_id(sql_text)='58u7kkx481c4x'
;
...
sql_handle => NULL,
plan_name  => 'SQL_PLAN_fzgwdzbvmmavx1f047357');
DBMS_OUTPUT.put_line('Plan has been dropped SUCCESSFULLY '||l_plans_dropped);
END;
select count(1) from V$SQLAREA where SQL_Id='5x7k7vzjkppss';

select count(*) from v$sql where sql_id ='5x7k7vzjkppss' and sql_plan_baseline is not null ;